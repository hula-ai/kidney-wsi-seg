...
def post_processing_for_each_batch_result(one_sample_result, wsi_h, wsi_w, wsi_c, patch_coordinates, kernel, classes, out_ratio, out_dim):
  value = random.random() * 5
  time.sleep(value)
  print('[multiproc] Start')

  # Initialize returned variables
  out_shape = (wsi_h, wsi_w, wsi_c)
  print('[multiproc] out_shape: ', out_shape)
  ...
  print('[multiproc] Start loop')
  print('[multiproc] len(one_sample_result): ', len(one_sample_result))
  print('[multiproc] type(one_sample_result): ', type(one_sample_result))

  list_of_patch_coordinates = []
  list_of_index_append = []
  list_of_class_segm_conf = []
  for class_id, (bbox, segm) in enumerate(zip(one_sample_result[0], one_sample_result[1])):
    ...
    list_of_patch_coordinates.append([])
    list_of_index_append.append([])
    list_of_class_segm_conf.append([])
    ...
    for class_bbox, class_segm in zip(bbox, segm):
      ...
      if np.any(class_segm):
        ...
        list_of_patch_coordinates[-1].append(patch_coordinates)
        list_of_index_append[-1].append(index_append)
        list_of_class_segm_conf[-1].append(class_segm_conf)

        print('[getsizeof] wsi_mask: ', getsizeof(wsi_mask))
        print('[getsizeof] patch_coordinates : ', getsizeof(patch_coordinates))
        print('[getsizeof] index_append: ', getsizeof(index_append))
        print('[getsizeof] class_segm_conf: ', getsizeof(class_segm_conf))
  ...
  return (class_prediction_confs, list_of_patch_coordinates, list_of_index_append, list_of_class_segm_conf)
...
def loop3(args, patches, GPU_BATCH_SIZE, inference_detector, model, tot_coords, kernel, wsi_mask, class_prediction_confs, CLASSES,
  ...
  pool_size = 100
  for s_id in range(0, len(patches), pool_size):
    if s_id + pool_size > len(patches):
      e_id = len(patches)
    else:
      e_id = s_id + pool_size
    ...
    with Pool(processes=25) as pool:
      ...
      for patch_id, one_sample_result in enumerate(all_batch_results[s_id:e_id]):
        a_func = pool.apply_async(post_processing_for_each_batch_result, [one_sample_result, wsi_h, wsi_w, wsi_c, tot_coords[patch_ids[patch_id+s_id]], kernel, CLASSES, OUT_RATIO, OUT_DIM])
        funclist.append(a_func)

      for func in funclist:
        an_mp_result = func.get(timeout=20)
    ...
    print('[**] multi processing, create final wsi_mask')
    # wsi_mask = results[-1][0]
    print('[**] multi processing, create final confs')

    for j, mp_result in enumerate(mp_results):
      if j == 0:
        class_prediction_confs = mp_result[0]
      else:
        for conf_key, conf_value in mp_result[0].items():
          class_prediction_confs[conf_key].extend(conf_value)

    for j, mp_result in enumerate(mp_results):
      cur_patch_coordinates = mp_result[1] 
      cur_index_append = mp_result[2]
      cur_class_segm_conf = mp_result[3]
      one_sample_result = all_batch_results[j]
      for class_id in range(len(one_sample_result[0])):
        if class_id > len(cur_index_append) - 1:
          break
        for bbox_mask_id in range(len(cur_index_append[class_id])):
          wsi_mask[int(cur_patch_coordinates[class_id][bbox_mask_id][1]//OUT_RATIO):int(cur_patch_coordinates[class_id][bbox_mask_id][1]//OUT_RATIO) + OUT_DIM,
          int(cur_patch_coordinates[class_id][bbox_mask_id][0]//OUT_RATIO):int(cur_patch_coordinates[class_id][bbox_mask_id][0]//OUT_RATIO) + OUT_DIM,
          class_id][cur_index_append[class_id][bbox_mask_id]] = cur_class_segm_conf[class_id][bbox_mask_id][cur_index_append[class_id][bbox_mask_id]].astype('float64') # note

  resized_wsi_mask = cv2.resize((wsi_mask*255).astype(np.uint8), (2048, 2048))
  cv2.imwrite('./curbest_global_mask.png', resized_wsi_mask)
...
def loop4(args, CLASSES, GT_MASKS_WSI_IDS, row, wsi_mask_cleaned, class_gt_tissue_masks,
  ...
  cv2.imwrite('./curbest_global_mask_cleaned_loop4.png', resized_wsi_mask)
...
def loop5(args, CLASSES, row, GT_MASKS_WSI_IDS, full_tissue_masking_canvas, wsi_mask_cleaned, class_gt_masks,
  ...
  resized_wsi_mask = cv2.resize((wsi_mask_cleaned*255).astype(np.uint8), (2048, 2048))
  cv2.imwrite('./curbest_global_mask_cleaned_loop5.png', resized_wsi_mask)
