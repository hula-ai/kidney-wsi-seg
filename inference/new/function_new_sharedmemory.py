...
def post_processing_for_each_batch_result(class_prediction_confs, one_sample_result, wsi_h, wsi_w, wsi_c, patch_coordinates, kernel, classes, out_ratio, out_dim, device):
  # value = random.random() * 5
  # time.sleep(value)

  # print('[multiproc] Start')
  # wsi_mask = np.frombuffer(mng_wsi_mask.get_obj()).reshape((wsi_h, wsi_w, wsi_c))
  shr_name = 'global_wsi_mask{}'.format(device)
  existing_shm = shared_memory.SharedMemory(create=False, name=shr_name)
  wsi_mask = np.ndarray((wsi_h, wsi_w, wsi_c), dtype=np.float64, buffer=existing_shm.buf)
...
def loop1(args, CLASSES, row, GT_MASKS_WSI_IDS, GT_MASKS, wsi_mask, SPECIAL_CLASS_CASES, OUT_RATIO,
  ...
  return class_gt_tissue_masks, class_roi_gt_masks, class_gt_masks

def loop2(args, tot_coords, wsi, IN_DIM, OUT_DIM, start_idx):
  # print('[In loop2] Start')
  patches = []
  next_idx = -1 # pause condition
  cnt = 0
  
  print('[loop2] len(tot_coords): ', len(tot_coords))
  for idx in range(start_idx, len(tot_coords)):
    # print('[loop2] len(tot_coords): ', len(tot_coords))
    coords = tot_coords[idx]
    ...
    cnt += 1
    if cnt == args.n_cml_patches:
      next_idx = idx + 1 
      print('[loop2] next_idx: ', next_idx)
      break

  # print('[In loop2] End')
  return patches, next_idx

def loop3(args, patches, GPU_BATCH_SIZE, inference_detector, model, tot_coords, kernel, wsi_mask, CLASSES,
          OUT_RATIO, OUT_DIM, device, start_idx, shared_class_prediction_confs, return_mask):
  ...
  try:
    ...
    shm = shared_memory.SharedMemory(create=True, size=wsi_mask.nbytes, name=shr_name)
    # shm.close()
    # shm.unlink()

  shared_wsi_mask = np.ndarray(wsi_mask.shape, dtype=wsi_mask.dtype, buffer=shm.buf)
  if start_idx == 0:
    shared_wsi_mask[:] = wsi_mask[:]
  print('[loop3] initialization time: ', time.time() - init_time)
  ...
  for i in range(0, len(patches), GPU_BATCH_SIZE):
    ...
    if (len(all_batch_results)%(args.n_mp_cml_eps*GPU_BATCH_SIZE) == 0) or ((i + GPU_BATCH_SIZE) > (len(patches) - 1)):
      start_time = time.time()
      patch_ids = list(range(0, len(all_batch_results)))

      # for patch_id, prediction in enumerate(batch_result):
      wsi_h, wsi_w, wsi_c = wsi_mask.shape
      print('[loop3] start of multi processing ')

      # update the values using our patch location coords
      # patch_coordinates = tot_coords.copy()[patch_ids[patch_id]]
      n_procs = multiprocessing.cpu_count() if args.n_mp_processes == -1 else args.n_mp_processes
      print('[loop3] setting number of processes for multi processing: ', n_procs)
      with closing(Pool(processes=int(n_procs/args.n_gpus))) as pool:
        funclist = []
        mp_results = []

        for patch_id, one_sample_result in enumerate(all_batch_results):
          a_func = pool.apply_async(post_processing_for_each_batch_result, args=(shared_class_prediction_confs, one_sample_result, wsi_h, wsi_w, wsi_c, tot_coords[patch_ids[patch_id] + start_i + start_idx], kernel, CLASSES, OUT_RATIO, OUT_DIM, device))
          funclist.append(a_func)

        for func in funclist:
          an_mp_result = func.get(timeout=100)
      ...
      all_batch_results.clear() # to release memory
      total_mp_time += time.time() - start_time
      ...
      # resized_wsi_mask = cv2.resize((wsi_mask*255).astype(np.uint8), (2048, 2048))
      # cv2.imwrite('./sharedmem_global_mask.png', resized_wsi_mask)
      start_i = None

  if return_mask is False:
    return None
  return_time = time.time()
  wsi_mask = shared_wsi_mask.copy()
  shm.close()
  shm.unlink()
  print('[loop3] return time: ', time.time() - return_time)

  print('~~~~~~~> Only inference time: {}'.format(total_inference_time))
  print('~~~~~~~> Only loop time: {}'.format(total_mp_time))
...
def loop6(args, CLASSES, row, GT_MASKS_WSI_IDS, wsi_mask_cleaned, class_gt_tissue_masks,
  ...
  return all_contours, aux_contours
        ...
        for saved_sub_ids, saved_sub_contours in enumerate(all_contours[CLASSES[sub_contour_index_id]]):
          ...
          if iou.item() > 0.0:
            ...
            if iou.item() > 0.5:
              ...
              cv2.imwrite(os.path.join(MULTI_DIR[CLASSES[contour_index_id]], row['sub_out_dir'], multi_file_name),
                          cv2.cvtColor(compartment, cv2.COLOR_RGB2BGR))
              cv2.imwrite(os.path.join(IMG_DIR[CLASSES[contour_index_id]], row['sub_out_dir'], img_file_name),
                          cv2.cvtColor(compartment_img, cv2.COLOR_RGB2BGR))
              if '.tif' in row['slide_id']:
                file_name_2 = row['slide_id'].split(' - Series ')[0] + '_' + \
                            CLASSES[contour_index_id] + 'OR' + CLASSES[sub_contour_index_id] + '_' + \
                            '({},{},{},{},{})-mask.png'.format(1.00, x, y, w, h)
                cv2.imwrite(os.path.join(MASK_DIR[CLASSES[contour_index_id]], row['sub_out_dir'], file_name), contour_canvas)
                cv2.imwrite(os.path.join(MASK_DIR[CLASSES[contour_index_id]], row['sub_out_dir'], file_name_2), contour_canvas)
              else:
                cv2.imwrite(os.path.join(MASK_DIR[CLASSES[contour_index_id]], row['sub_out_dir'], file_name), contour_canvas)
...
def loop13(args, CLASSES, all_contours, delete_ids, OUT_RATIO, row, wsi, IMG_DIR, MASK_DIR, MULTI_DIR):
  for class_names in CLASSES:
    for cont_id, contours in enumerate(all_contours[class_names]):
      ...
      compartment_img = wsi[img_y:img_y_end, img_x:img_x_end, :].copy()
      compartment[np.invert(contour_canvas.astype('bool'))] = 0
      cv2.imwrite(os.path.join(MULTI_DIR[class_names], row['sub_out_dir'], multi_file_name), cv2.cvtColor(compartment, cv2.COLOR_RGB2BGR))
      cv2.imwrite(os.path.join(IMG_DIR[class_names], row['sub_out_dir'], img_file_name),
                  cv2.cvtColor(compartment_img, cv2.COLOR_RGB2BGR))
      if '.tif' in row['slide_id']:
        file_name_2 = row['slide_id'].split(' - Series ')[0] + '_' + \
                      class_names + '_' + \
                      '({},{},{},{},{})-mask.png'.format(1.00, x, y, w, h)
        cv2.imwrite(os.path.join(MASK_DIR[class_names], row['sub_out_dir'], file_name), contour_canvas)
        cv2.imwrite(os.path.join(MASK_DIR[class_names], row['sub_out_dir'], file_name_2), contour_canvas)
      else:
        cv2.imwrite(os.path.join(MASK_DIR[class_names], row['sub_out_dir'], file_name), contour_canvas)
