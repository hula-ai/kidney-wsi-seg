...
def post_processing_for_each_batch_result(one_sample_result, wsi_h, wsi_w, wsi_c, tot_coords, patch_ids, patch_id, kernel, classes, out_ratio, out_dim):
  ...
  for class_id, (bbox, segm) in enumerate(zip(one_sample_result[0], one_sample_result[1])):
    ...
    for class_bbox, class_segm in zip(bbox, segm):
      ...
      if np.any(class_segm):
        ...
        mng_class_prediction_confs[classes[class_id]].append(conf_class_pred)
...
def init_worker(shared_wsi_mask, shared_class_prediction_confs):
  global mng_wsi_mask
...
def loop3(args, patches, GPU_BATCH_SIZE, inference_detector, model, tot_coords, kernel, wsi_mask, class_prediction_confs, CLASSES,
  ...
  # with Manager() as manager: # for sharing
  # manager = Manager()
  print('[**] multi processing, init confs')
  shared_class_prediction_confs = dict()
  shared_class_prediction_confs.update(class_prediction_confs)
  ...
  with Pool(processes=10, initializer=init_worker, initargs=(shared_wsi_mask, shared_class_prediction_confs,)) as pool:
    print('[**] multi processing, starmap')
    pool.starmap_async(post_processing_for_each_batch_result, [(one_sample_result, wsi_h, wsi_w, wsi_c, tot_coords, patch_ids, patch_id, kernel, CLASSES, OUT_RATIO, OUT_DIM) for patch_id, one_sample_result in enumerate(all_batch_results)])

    print('[**] multi processing, convert mng confs to pure confs')
    class_prediction_confs = dict(shared_class_prediction_confs)
    print('[**] multi processing, convert mng wsi_mask to pure wsi_mask')
    wsi_mask = np.frombuffer(shared_wsi_mask.get_obj()).reshape(wsi_h, wsi_w, wsi_c)
