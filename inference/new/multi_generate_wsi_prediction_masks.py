...
def inference_on_wsis(device, args, index, row, config_file, checkpoint_file, conf_mlp_norm_val_to_test, conf_mlp_norm_test_to_val, \
                      CLASSES, GPU_BATCH_SIZE, SPECIAL_CLASS_CASES, GT_MASKS, GT_MASKS_RAW_NAMES, GT_MASKS_WSI_IDS, IN_DIM, OUT_DIM, OUT_RATIO, \
                        FIG_SCALE, COLORS, WSI_PATH, WSI_SEG_PATH, mrxs_special_dir, process_df, PROJECT, SUB_DF_ID):
  # set dir names
  ...
  # === LOOP 2 ===
  start_time = time.time()
  patches = loop2(args, tot_coords, wsi, IN_DIM, OUT_DIM, patches)
...
if __name__ == '__main__':
  ...
  mp_arg_1 = [args for gpu_id in range(process_df.shape[0])]
  indices_list = []
  mp_rows = []
  for index, row in process_df.iterrows():
    indices_list.append(index)
    mp_rows.append(row)
  mp_arg_2 = [config_file for gpu_id in range(process_df.shape[0])]
  mp_arg_3 = [checkpoint_file for gpu_id in range(process_df.shape[0])]

  gpus_list = []
  for idx in range(process_df.shape[0]):
    gpus_list.append(idx%args.n_gpus)
  mp_arg_5 = [conf_mlp_norm_val_to_test for gpu_id in range(process_df.shape[0])]
  mp_arg_6 = [conf_mlp_norm_test_to_val for gpu_id in range(process_df.shape[0])]
  mp_arg_7 = [CLASSES for gpu_id in range(process_df.shape[0])]
  mp_arg_8 = [GPU_BATCH_SIZE for gpu_id in range(process_df.shape[0])]
  mp_arg_9 = [SPECIAL_CLASS_CASES for gpu_id in range(process_df.shape[0])]
  mp_arg_10 = [GT_MASKS for gpu_id in range(process_df.shape[0])]
  mp_arg_11 = [GT_MASKS_RAW_NAMES for gpu_id in range(process_df.shape[0])]
  mp_arg_12 = [GT_MASKS_WSI_IDS for gpu_id in range(process_df.shape[0])]
  mp_arg_13 = [IN_DIM for gpu_id in range(process_df.shape[0])]
  mp_arg_14 = [OUT_DIM for gpu_id in range(process_df.shape[0])]
  mp_arg_15 = [OUT_RATIO for gpu_id in range(process_df.shape[0])]
  mp_arg_16 = [FIG_SCALE for gpu_id in range(process_df.shape[0])]
  mp_arg_17 = [COLORS for gpu_id in range(process_df.shape[0])]
  mp_arg_18 = [WSI_PATH for gpu_id in range(process_df.shape[0])]
  mp_arg_19 = [WSI_SEG_PATH for gpu_id in range(process_df.shape[0])]
  mp_arg_20 = [mrxs_special_dir for gpu_id in range(process_df.shape[0])]
  mp_arg_21 = [process_df for gpu_id in range(process_df.shape[0])]
  mp_arg_22 = [PROJECT for gpu_id in range(process_df.shape[0])]
  mp_arg_23 = [SUB_DF_ID for gpu_id in range(process_df.shape[0])]
  ...
  with futures.ProcessPoolExecutor(max_workers=int(args.n_gpus)) as executor:
    ...
    indices_left = len(indices_list)
    indices_iter = iter(indices_list)

    while(indices_left):
      for this_index in indices_iter:
        job = executor.submit(inference_on_wsis, gpus_list[this_index], mp_arg_1[this_index], this_index, mp_rows[this_index], mp_arg_2[this_index], mp_arg_3[this_index], mp_arg_5[this_index], \
                              mp_arg_6[this_index], mp_arg_7[this_index], mp_arg_8[this_index], mp_arg_9[this_index], mp_arg_10[this_index], \
                              mp_arg_11[this_index], mp_arg_12[this_index], mp_arg_13[this_index], mp_arg_14[this_index], mp_arg_15[this_index], \
                              mp_arg_16[this_index], mp_arg_17[this_index], mp_arg_18[this_index], mp_arg_19[this_index], mp_arg_20[this_index], mp_arg_21[this_index], mp_arg_22[this_index], mp_arg_23[this_index])
        jobs[job] = this_index
      ...
      for job in futures.as_completed(jobs):
        indices_left -= 1 #one down - many to go...   <---
