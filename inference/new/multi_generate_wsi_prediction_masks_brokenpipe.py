...
def inference_on_wsis(device, args, config_file, checkpoint_file, conf_mlp_norm_val_to_test, conf_mlp_norm_test_to_val, \
                      CLASSES, GPU_BATCH_SIZE, SPECIAL_CLASS_CASES, GT_MASKS, GT_MASKS_RAW_NAMES, GT_MASKS_WSI_IDS, IN_DIM, OUT_DIM, OUT_RATIO, \
                        FIG_SCALE, COLORS, WSI_PATH, WSI_SEG_PATH, mrxs_special_dir, ori_process_df, PROJECT, SUB_DF_ID):
  # set dir names
  ...
  tissue_device = torch.device('cuda:{}'.format(device) if torch.cuda.is_available() else 'cpu')
  ...
  for index, row in ori_process_df.iterrows():
    process_df = ori_process_df.loc[[index], :]
    used_index = 0
    ...
    if '- macro' in row['slide_id']:
      ...
      process_df.loc[used_index, 'status'] = 'thumbnail'
      continue
      ...
      try:
        ...
        process_df.loc[used_index, 'status'] = 'failed_to_load'
        continue
    ...
    wsi_mask = loop3(args, patches, GPU_BATCH_SIZE, inference_detector, model, tot_coords, kernel, \
                    wsi_mask, class_prediction_confs, CLASSES, OUT_RATIO, OUT_DIM, device)
    ...
    loop4(args, CLASSES, GT_MASKS_WSI_IDS, row, wsi_mask_cleaned, class_gt_tissue_masks, process_df, \
          used_index, conf_mlp_norm_test_to_val, conf_mlp_norm_val_to_test, slide_wise_cutoffs)
    ...
    loop5(args, CLASSES, row, GT_MASKS_WSI_IDS, full_tissue_masking_canvas, wsi_mask_cleaned, class_gt_masks, \
          slide_wise_cutoffs, process_df, used_index, class_gt_tissue_masks, class_roi_gt_masks)
    ...
    # === LOOP 11 ===
    start_time = time.time()
    loop11(args, CLASSES, process_df, used_index, contour_counts, slide_wise_cutoffs, row, GT_MASKS_WSI_IDS, \
    ...
    process_df.loc[used_index, 'status'] = 'processed'
    process_df.loc[used_index, 'reprocess'] = 0
    # process_df.to_csv(WSI_SEG_PATH + 'process_list_{}_{}_gpu{}_{}.csv'.format(PROJECT, SUB_DF_ID, device, index), index=False)
...
if __name__ == '__main__':
  ...
  mp_arg_1 = [args for gpu_id in range(args.n_gpus)]
  mp_arg_2 = [config_file for gpu_id in range(args.n_gpus)]
  mp_arg_3 = [checkpoint_file for gpu_id in range(args.n_gpus)]
  gpus_list = [gpu_id for gpu_id in range(args.n_gpus)]
  mp_arg_5 = [conf_mlp_norm_val_to_test for gpu_id in range(args.n_gpus)]
  mp_arg_6 = [conf_mlp_norm_test_to_val for gpu_id in range(args.n_gpus)]
  mp_arg_7 = [CLASSES for gpu_id in range(args.n_gpus)]
  mp_arg_8 = [GPU_BATCH_SIZE for gpu_id in range(args.n_gpus)]
  mp_arg_9 = [SPECIAL_CLASS_CASES for gpu_id in range(args.n_gpus)]
  mp_arg_10 = [GT_MASKS for gpu_id in range(args.n_gpus)]
  mp_arg_11 = [GT_MASKS_RAW_NAMES for gpu_id in range(args.n_gpus)]
  mp_arg_12 = [GT_MASKS_WSI_IDS for gpu_id in range(args.n_gpus)]
  mp_arg_13 = [IN_DIM for gpu_id in range(args.n_gpus)]
  mp_arg_14 = [OUT_DIM for gpu_id in range(args.n_gpus)]
  mp_arg_15 = [OUT_RATIO for gpu_id in range(args.n_gpus)]
  mp_arg_16 = [FIG_SCALE for gpu_id in range(args.n_gpus)]
  mp_arg_17 = [COLORS for gpu_id in range(args.n_gpus)]
  mp_arg_18 = [WSI_PATH for gpu_id in range(args.n_gpus)]
  mp_arg_19 = [WSI_SEG_PATH for gpu_id in range(args.n_gpus)]
  mp_arg_20 = [mrxs_special_dir for gpu_id in range(args.n_gpus)]
  mp_arg_21 = [process_df for gpu_id in range(args.n_gpus)]
  mp_arg_22 = [PROJECT for gpu_id in range(args.n_gpus)]
  mp_arg_23 = [SUB_DF_ID for gpu_id in range(args.n_gpus)]

  MAX_JOBS_IN_QUEUE = 2
  with futures.ProcessPoolExecutor(max_workers=int(args.n_gpus)) as executor:
    jobs = {}
    
    gpus_left = len(gpus_list)
    gpus_iter = iter(gpus_list)

    while(gpus_left):
      for this_gpu in gpus_iter:
        job = executor.submit(inference_on_wsis, this_gpu, mp_arg_1[this_gpu], mp_arg_2[this_gpu], mp_arg_3[this_gpu], mp_arg_5[this_gpu], \
                              mp_arg_6[this_gpu], mp_arg_7[this_gpu], mp_arg_8[this_gpu], mp_arg_9[this_gpu], mp_arg_10[this_gpu], \
                              mp_arg_11[this_gpu], mp_arg_12[this_gpu], mp_arg_13[this_gpu], mp_arg_14[this_gpu], mp_arg_15[this_gpu], \
                              mp_arg_16[this_gpu], mp_arg_17[this_gpu], mp_arg_18[this_gpu], mp_arg_19[this_gpu], mp_arg_20[this_gpu], mp_arg_21[this_gpu], mp_arg_22[this_gpu], mp_arg_23[this_gpu])
        jobs[job] = this_gpu
      ...
      for job in futures.as_completed(jobs):
        gpus_left -= 1 #one down - many to go...   <---
  ...
  # import sys
  # print('=> Size of a dataframe'. sys.getsizeof(process_df))
            
  # with ProcessPoolExecutor(max_workers=int(args.n_gpus)) as executor:
    # list(executor.map(inference_on_wsis, mp_arg_1, mp_arg_2, mp_arg_3, mp_arg_4, mp_arg_5, \
    #                                                mp_arg_6, mp_arg_7, mp_arg_8, mp_arg_9, mp_arg_10, \
    #                                                mp_arg_11, mp_arg_12, mp_arg_13, mp_arg_14, mp_arg_15, \
    #                                                mp_arg_16, mp_arg_17, mp_arg_18, mp_arg_19, mp_arg_20, mp_arg_21, mp_arg_22, mp_arg_23))


    # for gpu_id in range(args.n_gpus):
    #   future = executor.submit(inference_on_wsis, mp_arg_1[gpu_id], mp_arg_2[gpu_id], mp_arg_3[gpu_id], mp_arg_4[gpu_id], mp_arg_5[gpu_id], \
    #                            mp_arg_6[gpu_id], mp_arg_7[gpu_id], mp_arg_8[gpu_id], mp_arg_9[gpu_id], mp_arg_10[gpu_id], \
    #                            mp_arg_11[gpu_id], mp_arg_12[gpu_id], mp_arg_13[gpu_id], mp_arg_14[gpu_id], mp_arg_15[gpu_id], \
    #                            mp_arg_16[gpu_id], mp_arg_17[gpu_id], mp_arg_18[gpu_id], mp_arg_19[gpu_id], mp_arg_20[gpu_id], mp_arg_21[gpu_id], mp_arg_22[gpu_id], mp_arg_23[gpu_id])
