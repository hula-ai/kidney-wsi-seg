...
def loop3(args, patches, GPU_BATCH_SIZE, inference_detector, model, tot_coords, kernel, wsi_mask, class_prediction_confs, CLASSES,
  ...
  mp_results = []
  pool_size = 30
  for s_proc_id in range(0, len(patches), pool_size):
    if s_proc_id + pool_size > len(patches):
      e_proc_id = len(patches)
    else:
      e_proc_id = s_proc_id + pool_size

    print('[**] multi processing, init Pool')
    with Pool(processes=5) as pool:
      print('[**] multi processing, starmap')
      partial_mp_results = pool.starmap(post_processing_for_each_batch_result, [(one_sample_result, wsi_h, wsi_w, wsi_c, tot_coords[patch_ids[patch_id+s_proc_id]], kernel, CLASSES, OUT_RATIO, OUT_DIM) \
                                                                                for patch_id, one_sample_result in enumerate(all_batch_results[s_proc_id:e_proc_id])])
      pool.close()
      pool.join()
      # pool.terminate()
    mp_results.extend(partial_mp_results)
    # break
